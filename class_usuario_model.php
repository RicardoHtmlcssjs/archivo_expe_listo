<?php
	...
	class Usuario extends Conexion{
		...
		public function mostar_cargo_espesifico($id_cargo){
			parent::conecta();
			$db = $this->conn;
			$query9 = $db->execute("SELECT desc_cargo FROM cargos WHERE id_cargo = '".$id_cargo."'");
		...
		// mostrar todos los archivos de subidos de los expedientes escaneados
		public function mostrar_expedientes_img($cedula){
			$conexion = new Conexion();
			$query1 = $conexion->consulta("s","SELECT * FROM expedientes_img WHERE fk_cedula = $cedula");
			$array = array();
			foreach($query1 as $key1){
				$array [] = $key1;
			}
